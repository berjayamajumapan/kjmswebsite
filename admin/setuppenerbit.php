...
if(isset($_POST['simpan'])){
	...
	if($_POST['no']=="AUTO"){ //tambah
		$no=ambilnomor("IDPenerbit","tr_penerbit","","000");
		$field[1][1]="IDPenerbit";
		$field[1][2]="'".$no."'";
		$field[2][1]="NamaPenerbit";
		$field[2][2]="'".$_POST['txt']."'";
		$field[3][1]="Keterangan";
		$field[3][2]="'".$_POST['MyTextAreaName']."'";
		$field[4][1]="";
		$field[4][2]="";
		generate_query("insert",$field,"tr_penerbit","","");
	}
	else{ //edit
		...
		$field[4][1]="";
		$field[4][2]="";
		generate_query("update",$field,"tr_penerbit","where IDPenerbit='".$_POST['no']."'","");
		if(isset($_GET['no'])){
			$_GET['no']="";
		}
	}
...
if(isset($_GET['hapus'])){
	if($_GET['no']!=""){
		generate_query("delete","","tr_penerbit","where IDPenerbit='".$_GET['no']."'","");
	}
	if(isset($_GET['no'])){
		$_GET['no']="";
	}
...
if(isset($_GET['edit'])){
	if($_GET['no']!=""){
		$result1= mysql_query("select * from tr_penerbit where IDPenerbit='".$_GET['no']."'");
		$row1=mysql_fetch_row($result1);
	}
}

$result = mysql_query("select * from tr_penerbit order by IDPenerbit");
...
?>
				...
				<tr>
					...
					<table class="tekshitam" cellspacing="0">
						<form action="setuppenerbit.php" method="post" enctype="multipart/form-data">
							<tr>
								<td>
									ID
								</td>
						...
						<table  bgcolor="#333333" cellspacing="1" border="0" width="80%" cellpadding="3">
							...
							<tr bgcolor="#FDFDFD" class="tekshitam" valign="top">
								...
								<td>
									<a href="setuppenerbit.php?edit=1&no=<?php echo $row[0]?>" class="link">Edit</a>
								</td>
								<td>
									<a href="setuppenerbit.php?hapus=1&no=<?php echo $row[0]?>" class="link">Delete</a>
								</td>
							</tr>
							<?php
							}
