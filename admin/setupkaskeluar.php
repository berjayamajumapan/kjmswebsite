...
if(isset($_POST['simpan'])){
        if($_POST['no']=="AUTO"){ //tambah
                $no=ambilnomor("ID","tr_kas_keluar","","000000");
                $field[1][1]="ID";
                $field[1][2]="'".$no."'";
                $field[2][1]="Tanggal";
                $field[2][2]="'".$_POST['tanggal']."'";
                $field[3][1]="DibayarKepada";
                $field[3][2]="'".$_POST['dari']."'";
                $field[4][1]="JumlahDibayar";
                ...
                $field[7][1]="";
                $field[7][2]="";
                generate_query("insert",$field,"tr_kas_keluar","","");
        }
        else{ //edit
                if($gambar=="")
                ...
                $field[7][1]="";
                $field[7][2]="";
                generate_query("update",$field,"tr_kas_keluar","where ID='".$_POST['no']."'","");
                if(isset($_GET['no'])){
                        $_GET['no']="";
                }
        }
...
if(isset($_POST['pub'])){
        $field[1][1]="Publish";
        $field[1][2]="'0'";
        generate_query("update",$field,"tr_kas_keluar","","");
        while(list($key, $value)=each($_POST['publish'])){
                $field[1][1]="Publish";
                $field[1][2]="'1'";
                generate_query("update",$field,"tr_kas_keluar","where ID='".$value."'","");
        }
}

if(isset($_GET['hapus'])){
        if($_GET['no']!=""){
                generate_query("delete","","tr_kas_keluar","where ID='".$_GET['no']."'","");
        }
        if(isset($_GET['no'])){
                $_GET['no']="";
        }
...
if(isset($_GET['edit'])){
        if($_GET['no']!=""){
                $result1= mysql_query("select * from tr_kas_keluar where ID='".$_GET['no']."'");
                $row1=mysql_fetch_row($result1);
        }
}

$result = mysql_query("select * from tr_kas_keluar order by ID desc");
...
?>
                                ...
                                <tr>
                                        ...
                                        <table class="tekshitam" cellspacing="0">
                                                <form action="setupkaskeluar.php" method="post" enctype="multipart/form-data">
                                                        <tr>
                                                                <td>
                                                                        ID
                                                                </td>
                                                ...
                                                <table  bgcolor="#333333" cellspacing="1" border="0" width="80%">
                                                        ...
                                                        <tr bgcolor="#FDFDFD" class="tekshitam" valign="top">
                                                                ...
                                                                <td>
                                                                        <a href="setupkaskeluar.php?edit=1&no=<?php echo $row[0]?>" class="link">Edit</a>
                                                                </td>
                                                                <td>
                                                                        <a href="setupkaskeluar.php?hapus=1&no=<?php echo $row[0]?>" class="link">Delete</a>
                                                                </td>
                                                                <td>
