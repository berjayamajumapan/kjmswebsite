...
if(isset($_POST['simpan'])){
        ...
        if (is_uploaded_file($_FILES['icon']['tmp_name'])){
                if (move_uploaded_file($_FILES['icon']['tmp_name'], "../cimages/".$_FILES['icon']['name']) ){
                        $error = "Success upload images...";
                        $suksesupload=1;
                        $icon="cimages/".$_FILES['icon']['name'];
                }
        }
        if($_POST['no']=="AUTO"){ //tambah
                $no=ambilnomor("IDItem","t_shop","","0000");
                $field[1][1]="IDItem";
                $field[1][2]="'".$no."'";
                $field[2][1]="Judul";
                ...
                $field[6][1]="";
                $field[6][2]="";
                generate_query("insert",$field,"t_shop","","");
        }
        else{ //edit
                if($gambar=="")
                ...
                $field[6][1]="";
                $field[6][2]="";
                generate_query("update",$field,"t_shop","where IDItem='".$_POST['no']."'","");
                if(isset($_GET['no'])){
                        $_GET['no']="";
                }
        }
...
if(isset($_POST['pub'])){
        $field[1][1]="Publish";
        $field[1][2]="'0'";
        generate_query("update",$field,"t_shop","","");
        while(list($key, $value)=each($_POST['publish'])){
                $field[1][1]="Publish";
                $field[1][2]="'1'";
                generate_query("update",$field,"t_shop","where IDItem='".$value."'","");
        }
}

if(isset($_GET['hapus'])){
        if($_GET['no']!=""){
                generate_query("delete","","t_shop","where IDItem='".$_GET['no']."'","");
        }
        if(isset($_GET['no'])){
                $_GET['no']="";
        }
...
if(isset($_GET['edit'])){
        if($_GET['no']!=""){
                $result1= mysql_query("select * from t_shop where IDItem='".$_GET['no']."'");
                $row1=mysql_fetch_row($result1);
        }
}

$result = mysql_query("select * from t_shop order by IDItem");
                                ...
                                <tr>
                                        <td>
                                                <table class="tekshitam" cellspacing="0">
                                                        <form action="setupshop.php" method="post" enctype="multipart/form-data">
                                                        <tr>
                                                                <td>
                                                                        ID Item
                                                                </td>
                                                ...
                                                <table  bgcolor="#333333" cellspacing="1" border="0" width="80%">
                                                        ...
                                                        <tr bgcolor="#FDFDFD" class="tekshitam" valign="top">
                                                                ...
                                                                <td>
                                                                        <a href="setupshop.php?edit=1&no=<?php echo $row[0]?>" class="link">Edit</a>
                                                                </td>
                                                                <td>
                                                                        <a href="setupshop.php?hapus=1&no=<?php echo $row[0]?>" class="link">Delete</a>
                                                                </td>
                                                                <td>
