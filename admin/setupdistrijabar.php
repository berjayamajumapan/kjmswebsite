...
if(isset($_POST['simpan'])){
	...
	if($_POST['no']=="AUTO"){ //tambah
	   $no=ambilnomor("ID","t_jabar","","000");
		...
		$field[6][1]="";
		$field[6][2]="";
		generate_query("insert",$field,"t_jabar","","");
	}
	else{ //edit
		if($gambar=="")
		...
		$field[6][1]="";
		$field[6][2]="";
		generate_query("update",$field,"t_jabar","where ID='".$_POST['no']."'","");
		if(isset($_GET['no'])){
			$_GET['no']="";
		}
	}
...
if(isset($_POST['pub'])){
	$field[1][1]="Publish";
	$field[1][2]="'0'";
	generate_query("update",$field,"t_jabar","","");
	while(list($key, $value)=each($_POST['publish'])){
		$field[1][1]="Publish";
		$field[1][2]="'1'";
		generate_query("update",$field,"t_jabar","where ID='".$_GET['no']."'","");
	}
}

if(isset($_GET['hapus'])){
	if($_GET['no']!=""){
		generate_query("delete","","t_jabar","where ID='".$_GET['no']."'","");
	}
	if(isset($_GET['no'])){
		$_GET['no']="";
	}
...
if(isset($_GET['edit'])){
	if($_GET['no']!=""){
		$result1= mysql_query("select * from t_jabar where ID='".$_GET['no']."'");
		$row1=mysql_fetch_row($result1);
	}
}

$result = mysql_query("select * from t_jabar order by ID");

header_view();
?>
				<tr>
					<td class="judulbiru">
						Setup Distributor Jabar:
					</td>
				</tr>
				<tr>
					<td>
						<table class="tekshitamkecil" cellspacing="0">
							<form action="setupdistrijabar.php" method="post" enctype="multipart/form-data">
							<tr>
								<td width="140">
									ID 
								</td>
						...
						<table  bgcolor="#333333" cellspacing="1" border="0" width="100%">
							...
							<tr bgcolor="#FDFDFD" class="tekshitam" valign="top">
								...
								<td>
									<a href="setupdistrijabar.php?edit=1&no=<?php echo $row[0]?>" class="link">Edit</a>
								</td>
								<td>
									<a href="setupdistrijabar.php?hapus=1&no=<?php echo $row[0]?>" class="link">Delete</a>
								</td>
								<td>
